...
public class ShopRequestController
{
    ...
    {
        var req = RestController.PatchRequestMessage($"{RequestsCompositeRoot.BASE_URL}game/currency/perform_click");
        req.SendWebRequest();
        while (!req.isDone) { }
    ...
    {
        var req = RestController.PatchRequestMessage($"{RequestsCompositeRoot.BASE_URL}game/currency/complete_cycle");
        req.SendWebRequest();
        while (!req.isDone) { }
        return JsonConvert.DeserializeObject<PlayerMoneyResponse>(req.downloadHandler.text);
    }
    ...
    {
        var req = RestController.GetRequestMessage($"{RequestsCompositeRoot.BASE_URL}game/shop/production/get_production_templates");
        req.SendWebRequest();
        while (!req.isDone) { }
    ...
    {
        var req = RestController.GetRequestMessage($"{RequestsCompositeRoot.BASE_URL}game/shop/production/get_player_production");
        req.SendWebRequest();
        while (!req.isDone) { }
    ...
    {
        var req = RestController.PatchRequestMessage($"{RequestsCompositeRoot.BASE_URL}game/shop/production/upgrade_production/{uid}");
        req.SendWebRequest();
        while (!req.isDone) { }
